...
class Reminder {
    ...
    public func createReminder(setDate: Date) {
        ...
        let statusEvent = EKEventStore.authorizationStatus(for: EKEntityType.event)
        let statusReminder = EKEventStore.authorizationStatus(for: EKEntityType.reminder)
        if ((statusEvent == EKAuthorizationStatus.authorized) && (statusReminder == EKAuthorizationStatus.authorized)) {
            ...
            eventStoreFastList.fetchReminders(matching: predicate, completion: { (reminders: [EKReminder]?) -> Void in
                ...
                if reminders.count != 0 {
                    ...
                    for reminder:EKReminder in reminders {
                        let reminderFastListTitle = "FastList is Updated"
                        if reminder.title == reminderFastListTitle {
